...
jest.mock('../../middlewares/mutler.config', () => ({
  FileMiddleware: jest.fn((req, res, next) => next())
}));
...
jest.mock('../../services/uploadImg', () => ({
  handleImageUpload: jest.fn().mockResolvedValue('/uploads/profile/test-profile.jpg')
}));
